...
import math
from spider_report import spider_report_control,get_page_count,get_article_url_list,get_report_message
from base_data_message import get_base_report_type_url
from report_pdf import download_pdf,reload_pdf
from db_mysql import mysql_connect,insert_mysql_t_finance_report,get_mysql_record
...
if __name__ == '__main__':
    ...
    while not thread_stop:
        ...
        while ((report_class_index < report_class_len) and (not thread_stop)):
            # print "test..."
            # current_date = time.strftime("%Y%m%d %H:%M:%S", time.localtime())
            # print current_date
            ...
            if page_count>0:
                ...
                page_url_index = 0
                while ((page_url_index<page_url_list_len) and (not thread_stop)):
                            ...
                            if(not thread_stop):
                                ...
                                if last_publish_date > datetime.datetime.strptime(report_element['time'], "%Y-%m-%d %H:%M:%S"):
                                    ...
                                    if pdf_url != '':
                                        ...
                                        file_name = report_element['file_name'] + '.'+ pdf_url.split('.')[-1]
                                        ...
                                        if download_result:
                                            ...
                                            if page_count == 0 and file_size == 0:
                                                ...
                                                if mysql_conn == 0:
                                                    ...
                                                    pdf_url = pdf_url
                                                    insert_mysql_t_finance_report(mysql_conn,report_id, publish_date, report_type_id, report_type, report_title,report_organization, file_size, page_count, file_url, pdf_url, report_content, file_local_path)
                                                    print "insert sucess!"
                                                #存储mysql数据库
                                        else:
